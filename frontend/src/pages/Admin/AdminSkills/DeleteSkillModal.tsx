...
export default function DeleteSkillModal({ isOpen, onClose, skill }: DeleteSkillModalProps) {
  const queryClient = useQueryClient()

  // Delete skill mutation
  const deleteSkillMutation = useMutation({
    mutationFn: (skillId: string) => adminApi.deleteSkill(skillId),
    onSuccess: () => {
      // Invalidate skills query để refresh danh sách
      queryClient.invalidateQueries({ queryKey: ['skills'] })
      // Show success message
      alert(`✅ Skill "${skill?.skill_name}" has been deleted successfully!`)
      // Close modal
      onClose()
    },
    onError: () => {
  ...
  const handleDelete = () => {
    if (skill) {
      deleteSkillMutation.mutate(skill.skill_id.toString())
    }
  }
    ...
    <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50'>
      <div className='bg-white rounded-lg shadow-xl w-full max-w-md mx-4'>
        ...
        <div className='p-6 space-y-4'>
          <p className='text-gray-700'>Are you sure you want to delete this skill?</p>

          {/* Skill Info Card */}
          <div className='bg-gray-50 rounded-lg p-4 border border-gray-200'>
            <div className='space-y-2'>
              <div className='flex items-center justify-between'>
                <span className='text-sm text-gray-600'>Skill ID:</span>
                <span className='text-sm font-medium text-gray-900'>{skill.skill_id}</span>
              </div>
              <div className='flex items-center justify-between'>
              ...
              {skill.s_description && (
                <div className='pt-2 border-t border-gray-200'>
                  ...
                  <p className='text-sm text-gray-700 mt-1'>{skill.s_description}</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className='flex items-start gap-2 p-3 bg-red-50 border border-red-200 rounded-lg'>
            ...
            <p className='text-sm text-red-800'>
              This skill will be permanently deleted. All jobs and candidates associated with this skill may be affected.
            </p>
          </div>
        </div>
